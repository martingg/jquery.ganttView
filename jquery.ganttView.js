...
(function (jQuery) {
  ...
  function build(options) {
	...
	var Chart = function(div, opts) {
      ...
      // create the top calendar header
      ...
      // create the display for the data
    ...
    function addGrid(div, data, dates, cellWidth, showWeekends) {
        ...
        // copy a row div for each piece of data
        for (var i = 0; i < data.length; i++) {
            for (var j = 0; j < data[i].series.length; j++) {
                gridDiv.append(rowDiv.clone().addClass("foldable-"+data[i].id));
            }
        }
        div.append(gridDiv);
    }
    ...
    function addBlockContainers(div, data) {
        ...
        for (var i = 0; i < data.length; i++) {
            for (var j = 0; j < data[i].series.length; j++) {
                blocksDiv.append(jQuery("<div>", { "class": "ganttview-block-container foldable-"+data[i].id }));
            }
        }
